...
$(OUTPUT_QCOW2): $(RAW_TAR)
	...
	virt-customize $(CUSTOMIZATION_ARGS) -a $(OUTPUT_QCOW2) \
		...
		--hostname "taler-debian" \
		--firstboot-command 'ssh-keygen -A && systemctl restart sshd' #\
		#--copy-in "debian_extra_packages/rcluster-mgmtd_0.1~bookworm.deb:/var/cache/apt/" \
		#--install "/var/cache/apt/rcluster-mgmtd_0.1~bookworm.deb"

$(IMAGEEROFS): $(OUTPUT_QCOW2)
	rm -rf rootfs_building
	mkdir -p rootfs_building
	guestfish --ro --format=qcow2 -m /dev/sda -a $(OUTPUT_QCOW2) copy-out / rootfs_building
	#guestfish --ro --format=qcow2 -m /dev/sda:/::ext2 -a $(OUTPUT_QCOW2) -i copy-out / rootfs_building
...
# this is so nix can override it to $out to instal there directly and get
# it recorded in the nix store

DESTDIR ?= ./output

# TODO split the build and install phases so that nix can make 3 drvs
# and caching works better

install:
	@mkdir -p "$DESTDIR"
	@install -m 444 $(IMAGEEROFS) $(DESTDIR)
